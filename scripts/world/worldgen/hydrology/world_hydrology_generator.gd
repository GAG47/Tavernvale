...
static func generate(
	...
	layer.local_runoff = _calculate_local_runoff(
		graph, preliminary_climate.precipitation, actual_settings.runoff_modifier
	)
	if layer.local_runoff.is_empty():
		return null
	layer.flow_to = _calculate_flow_to(graph, terrain.terrain_height, closed_basin_id)
	if layer.flow_to.is_empty():
		return null
	var topology := _accumulate_flow(layer.local_runoff, layer.flow_to)
	if not topology.ok:
...
static func _calculate_local_runoff(
		graph: SpatialGraph,
	...
	for cell_id in graph.cell_count():
		...
		if not is_finite(runoff[cell_id]) or runoff[cell_id] < 0.0:
			push_error("Formal Hydrology local runoff overflowed Float32 storage")
			return PackedFloat32Array()
	return runoff


static func _calculate_flow_to(
		graph: SpatialGraph,
	...
	for cell_id in graph.cell_count():
		...
		if best_neighbor < 0:
			push_error("Formal Hydrology found an unconditioned Sink at Cell %d" % cell_id)
			return PackedInt32Array()
		flow_to[cell_id] = best_neighbor
	return flow_to


static func _accumulate_flow(
