...
const DEBUG_PAGE_VIEWS := [
	...
	[
		ViewMode.AGRICULTURE_POTENTIAL,
		ViewMode.TIMBER_POTENTIAL,
		ViewMode.FORAGE_POTENTIAL,
		ViewMode.CONSTRUCTION_STONE_POTENTIAL,
		ViewMode.BASE_METAL_POTENTIAL,
...
func _cell_color(cell_id: int) -> Color:
	match _map_view_mode():
		...
		ViewMode.AGRICULTURE_POTENTIAL:
			return _resource_heatmap_color(resource_potential.agriculture_potential[cell_id])
		ViewMode.TIMBER_POTENTIAL:
			return _resource_heatmap_color(resource_potential.timber_potential[cell_id])
		ViewMode.FORAGE_POTENTIAL:
			return _resource_heatmap_color(resource_potential.forage_potential[cell_id])
		ViewMode.CONSTRUCTION_STONE_POTENTIAL:
			return _resource_heatmap_color(resource_potential.construction_stone_potential[cell_id])
		ViewMode.BASE_METAL_POTENTIAL:
			return _resource_heatmap_color(resource_potential.base_metal_potential[cell_id])
		ViewMode.PRECIOUS_MINERAL_POTENTIAL:
			return _resource_heatmap_color(resource_potential.precious_mineral_potential[cell_id])
		ViewMode.FRESHWATER_AQUATIC_POTENTIAL:
...
func _resource_heatmap_color(value: float) -> Color:
	...
	if normalized < 0.33:
		return Color(0.055, 0.075, 0.12).lerp(Color(0.12, 0.32, 0.48), normalized / 0.33)
	if normalized < 0.67:
		return Color(0.12, 0.32, 0.48).lerp(
			Color(0.12, 0.68, 0.40), (normalized - 0.33) / 0.34
		)
...
func _regenerate_composition() -> void:
	...
	started = Time.get_ticks_msec()
	geology = null if projected_terrain == null else GeologyGenerator.generate(
		graph, composition, projected_terrain
	)
	...
	started = Time.get_ticks_msec()
	resource_settings = ResourcePotentialSettings.new()
	resource_potential = null if soil == null else ResourcePotentialGenerator.generate(
		graph,
		terrain,
		climate,
		formal_hydrology,
...
func _draw_information() -> void:
	...
	var lines := PackedStringArray([
		"Tavernvale World Generation Debug",
...
func _append_resource_cell_inspection(lines: PackedStringArray, cell_id: int) -> void:
	...
	lines.append("Forage: %.4f" % resource_potential.forage_potential[cell_id])
	...
	if view_mode == ViewMode.AGRICULTURE_POTENTIAL:
		var slope_factor := SoilGenerator.slope_factor_for(
			graph, terrain.terrain_height, cell_id, soil_settings.slope_reference
		)
		var texture_factor := ResourcePotentialGenerator.texture_factor_for(soil.soil_texture_id[cell_id])
		var soil_quality := 0.50 * soil.soil_fertility[cell_id] \
				+ 0.30 * soil.soil_depth[cell_id] + 0.20 * texture_factor
		var temperature_suitability := smoothstep(0.0, 10.0, climate.temperature[cell_id]) * (
			1.0 - smoothstep(30.0, 40.0, climate.temperature[cell_id])
		)
		lines.append("Soil Quality: %.4f" % soil_quality)
		lines.append("Temperature Suitability: %.4f" % temperature_suitability)
		lines.append("Water Suitability: %.4f" % smoothstep(
			0.15, 0.40, ecology.ecological_moisture[cell_id]
		))
		lines.append("Terrain Suitability: %.4f" % (
		...
		lines.append("Concentration: %.4f" % ResourcePotentialGenerator.precious_mineral_concentration_for(
			precious_raw
		))
	elif view_mode == ViewMode.FRESHWATER_AQUATIC_POTENTIAL:
		var strength := 0.0
		if formal_hydrology.is_river(cell_id):
			strength = EcologyGenerator.river_strength_for(
				formal_hydrology.flow_accumulation[cell_id], formal_hydrology.settings.river_runoff_threshold
			)
		lines.append("Formal River Strength: %.4f" % strength)
		lines.append("Lake Habitat: %s" % ("Yes" if surface_water.lake_id[cell_id] >= 0 else "No"))
		lines.append("Temperature Suitability: %.4f" % ResourcePotentialGenerator.freshwater_temperature_suitability_for(
		...
		lines.append("Estuary Strength: %.4f" % ResourcePotentialGenerator.estuary_strength_for(
			graph, terrain, formal_hydrology, cell_id
		))
		lines.append("Temperature Suitability: %.4f" % ResourcePotentialGenerator.coastal_temperature_suitability_for(
			climate.temperature[cell_id]
		))
...
func _calculate_resource_statistics() -> Dictionary:
	var result := {}
	for mode in DEBUG_PAGE_VIEWS[DebugPage.RESOURCES]:
		var values := PackedFloat32Array()
		var source := _resource_values_for_mode(mode)
		for cell_id in graph.cell_count():
...
func _resource_continuous_statistics(values: PackedFloat32Array) -> Dictionary:
	...
	for threshold in [0.25, 0.50, 0.75]:
		...
		for value in values:
			if value >= threshold:
				threshold_count += 1
		statistics["ge_%d" % int(threshold * 100.0)] = threshold_count
	return statistics


func _append_resource_statistics(lines: PackedStringArray, statistics: Dictionary) -> void:
	if statistics.is_empty():
		lines.append("No eligible Cells")
		return
	var count: int = statistics.count
	lines.append("Denominator: %s (%d Cells)" % [statistics.denominator_name, count])
	lines.append("Mean: %.4f" % statistics.mean)
	lines.append("P25: %.4f" % statistics.p25)
	lines.append("P50 / Median: %.4f" % statistics.p50)
	lines.append("P75: %.4f" % statistics.p75)
	...
	lines.append("P95: %.4f" % statistics.p95)
	lines.append("Max: %.4f" % statistics.max)
	for threshold_key in ["ge_25", "ge_50", "ge_75"]:
		var threshold_count: int = statistics[threshold_key]
		lines.append(
			">= %.2f: %d / %.2f%%"
...
func _resource_values_for_mode(mode: int) -> PackedFloat32Array:
	match mode:
		ViewMode.AGRICULTURE_POTENTIAL:
			return resource_potential.agriculture_potential
		ViewMode.TIMBER_POTENTIAL:
			return resource_potential.timber_potential
		ViewMode.FORAGE_POTENTIAL:
			return resource_potential.forage_potential
		ViewMode.CONSTRUCTION_STONE_POTENTIAL:
			return resource_potential.construction_stone_potential
		ViewMode.BASE_METAL_POTENTIAL:
			return resource_potential.base_metal_potential
		ViewMode.PRECIOUS_MINERAL_POTENTIAL:
			return resource_potential.precious_mineral_potential
		ViewMode.FRESHWATER_AQUATIC_POTENTIAL:
			return resource_potential.freshwater_aquatic_potential
		ViewMode.COASTAL_AQUATIC_POTENTIAL:
			return resource_potential.coastal_aquatic_potential
		_:
			return PackedFloat32Array()


func _resource_denominator_includes(mode: int, cell_id: int) -> bool:
	if mode == ViewMode.FRESHWATER_AQUATIC_POTENTIAL:
		return surface_water.lake_id[cell_id] >= 0 or formal_hydrology.is_river(cell_id)
	if mode == ViewMode.COASTAL_AQUATIC_POTENTIAL:
		return ResourcePotentialGenerator.is_coastal_ocean_cell(graph, terrain, cell_id)
	return terrain.terrain_height[cell_id] >= 0.0 and surface_water.lake_id[cell_id] < 0


func _resource_denominator_name(mode: int) -> String:
	if mode == ViewMode.FRESHWATER_AQUATIC_POTENTIAL:
		return "River + Lake Habitat"
	if mode == ViewMode.COASTAL_AQUATIC_POTENTIAL:
		return "Coastal Ocean"
	return "Land (no Lakes)"
...
func _current_page_views() -> Array:
	return DEBUG_PAGE_VIEWS[debug_page]


func _change_debug_page(offset: int) -> void:
	debug_page = posmod(debug_page + offset, DEBUG_PAGE_VIEWS.size())
	var page_views := _current_page_views()
	view_mode = page_views[0]


func _select_current_page_view(view_index: int) -> void:
	var page_views := _current_page_views()
...
func _ensure_valid_page_view() -> void:
	var page_views := _current_page_views()
	if not view_mode in page_views:
		view_mode = page_views[0]
