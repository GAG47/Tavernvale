...
static func generate(
	...
	for cell_id in count:
		...
		for logical_layer_index in sequence.size():
			strata.rock_type_ids.append(sequence[logical_layer_index])
			strata.top_z.append(current_top_z)
			if logical_layer_index + 1 < sequence.size():
	...
	var validation_errors := SubsurfaceStrataValidator.validate(
		graph, terrain, geology, strata, world_seed, actual_settings
	)
...
static func _inputs_are_valid(
	...
	if graph == null or terrain == null or geology == null:
		push_error(
			"Subsurface Strata requires SpatialGraph, Final Terrain, and Geology"
		)
		return false
	var count := graph.cell_count()
